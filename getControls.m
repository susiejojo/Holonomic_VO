function controls = getControls(agent_pos,agent_goal,agent_rad,sensedObstacles,obst_rad,vmax,time_sample)
    init = [0 0];
    A = [];
    b = [];
    Aeq = [];
    ...
    if size(sensedObstacles,1)>0
        constraints = @(agent_velo)getConstraints(agent_velo,agent_pos,sensedObstacles(1,:),agent_rad,obst_rad,time_sample);
